...
let h=`<div class="profile-card"><div class="profile-pic-wrap" onclick="checkPhotoPrivacy(()=>document.getElementById('picInput').click())">${userData.profilePic?'<img src="'+userData.profilePic+'">':'<div class="pp-placeholder">👤</div>'}</div> <div class="profile-name">@${userData.username||'???'}</div> <div class="profile-realname">${userData.name||''} <span style="font-size:.6rem;color:var(--dim)">(only you see this)</span></div> <div class="profile-class">${userData.class}${userData.subclass?' — '+userData.subclass:''}</div> <div class="profile-rank-badge" style="color:${r.color};background:${r.color}22;border:1px solid ${r.color}44">${r.name} — ${title}</div> ${userData.bio?'<div class="profile-bio">'+esc(userData.bio)+'</div>':''} <div class="xp-bar-wrap"><div class="xp-bar-label"><span>${info.label}</span><span>${info.sublabel}</span></div><div class="xp-bar"><div class="xp-bar-fill" style="width:${info.pct}%;background:linear-gradient(90deg,${r.color},${r.color}cc)"></div></div></div></div>`;
// Stats
...
const req=doc.data();
h+=`<div class="friend-request"> <div class="lb-pic">${req.fromPic?'<img src="'+req.fromPic+'">':'👤'}</div> <div class="lb-info"><div class="lb-name">@${esc(req.fromUsername)}</div><div class="lb-class">wants to be friends</div></div> <button class="fr-accept" onclick="acceptFriendRequest('${doc.id}','${req.from}')">✓</button> <button class="fr-decline" onclick="declineFriendRequest('${doc.id}')">✕</button> </div>`;
}
el.innerHTML=h;
}catch(e){
...
let h=`<div class="pc-header"> <div class="pc-pic">${u.profilePic?'<img src="'+u.profilePic+'">':'<div class="pp-placeholder" style="width:60px;height:60px;font-size:1.6rem">👤</div>'}</div> <div class="pc-info"> <div class="pc-username">@${esc(u.username||'hunter')}</div> <div class="pc-class">${u.class||''}${u.subclass?' — '+u.subclass:''}</div> <div class="pc-rank" style="color:${r.color}">${r.name} — ${title}</div> </div> </div>`;
if(u.bio)h+=`<div class="pc-bio">${esc(u.bio)}</div>`;
h+=`<div class="pc-stats"> <div class="pc-stat"><div class="pc-stat-val">${u.xp||0}</div><div class="pc-stat-label">XP</div></div> <div class="pc-stat"><div class="pc-stat-val">${u.workouts||'?'}</div><div class="pc-stat-label">Workouts</div></div> <div class="pc-stat"><div class="pc-stat-val">${(u.achievements||[]).length}</div><div class="pc-stat-label">Achieve.</div></div> </div>`;
if(isFriend){
...
h+=`<div class="client-card" onclick="openClientDetail('${uid}')"> <div class="lb-pic">${u.profilePic?'<img src="'+u.profilePic+'">':'👤'}</div> <div class="client-info"> <div class="client-name">@${esc(u.username||'hunter')} <span class="client-status">${status}</span></div> <div class="client-stats">${lifts} lifts · ${activities} cardio · ${rest} rest · ${weighIns} check-ins</div> <div class="client-goal">${esc(u.nutritionGoal||u.goal||'No goal set')}</div> </div> </div>`;
}catch(e){}
}
...
h+=`<div class="photo-card" onclick="viewPhoto(${p.id})"> <img src="${p.dataUrl}" alt="Progress"> <div class="photo-meta"> <div class="photo-date">${dt}</div> <div class="photo-pose">${poseEmoji} ${p.pose}${weight}</div> </div> <button class="photo-del" onclick="event.stopPropagation();deletePhoto(${p.id})">✕</button> </div>`;
});
...
$(‘photoCompareBody’).innerHTML=` <div class="compare-grid"> <div class="compare-col"><img src="${first.dataUrl}"><div class="compare-cap">${d1}<br>${first.weight?first.weight+' lbs':''}</div></div> <div class="compare-col"><img src="${latest.dataUrl}"><div class="compare-cap">${d2}<br>${latest.weight?latest.weight+' lbs':''}</div></div> </div> <div class="compare-summary">${daysBetween} days${weightDiff!==null?' · '+(weightDiff>0?'+':'')+weightDiff+' lbs':''}</div>`;
